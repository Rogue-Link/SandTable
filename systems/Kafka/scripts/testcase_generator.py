    ...
    for i, msg, cur_state in zip(jq_trace.input(states), jq_msgs.input(states), states):
        ...
        if len(i) > 1:
            ...
            if cmd == 'conn_part_flush':
                ...
                if comment[0] == 'HandleSuccessFetchResponse':
                    yield ['sleep', '1000']
                elif comment[0] == 'HandleDivergingFetchResponse':
                    yield ['sleep', '1000']
                elif comment[0] == 'HandleErrorFetchResponse':
                    yield ['sleep', '1000']
                #yield from compare(parameters[0])
            elif cmd == 'msg_add':  # send msg (enqueued in controller)
                if comment[0] == 'SendFetchRequest':
                    yield ['sleep', '1000']
                ...
                if comment[0] == 'HandleMsgRVR':
                    yield ['sleep', '6000']
                elif comment[0] == 'RejectFetchRequest':
                    yield ['sleep', '4000']
                elif comment[0] == 'DivergingFetchRequest':
                    yield ['sleep', '4000']
                elif comment[0] == 'AcceptFetchRequest':
                    yield ['sleep', '4000']
                #yield from compare(parameters[0])
                ...
                if comment[0] == 'ElectionTimeout':
                    yield ['intercept', comment[1], 'inc_time_ms', '1850']  # < RequestTimeout
                yield from do_tick(comment[1])
                yield ['intercept', comment[1], 'inc_time_ms', '101']   # skip backoff time
                yield ['sleep', '5000']
                ...
                yield ['sleep', '1000']
                yield ['intercept', comment[1], 'inc_time_ms', '300'] 
        else:
            if comment[0] == 'init':
                yield ['init', str(i[0][1])]
                yield ['sleep', '180000']  # sleep 5 seconds to wait nodes init
                # for j in nodes:

...
def parse_args():
    global cli_path
    parser = argparse.ArgumentParser(description="Test case generator for Kafka")
    ...
    parser.add_argument('-p', dest='path', action='store', required=False, help='Kafka path',
                        default=None)
...
def yield_trace(states):
    def compare(n, code_var_name, model_var_name, no_compare=False):
        pass  # do not compare in Etcd
    def do_tick(n, is_compare=True):
        yield ['sleep', '1000']  # sleep 1s to wait the node processing
    def deliver(src, dst):
        ...
        try:
            if prev_state is not None:
                if prev_state['state'][dst] != cur_state['state'][dst]:
                    ...
                    yield ['#', 'variable', 'currentEpoch', dst, str(cur_state['currentEpoch'][dst])]
                    yield ['intercept', dst, 'state_get', 'currentEpoch']
                    yield ['compare', 'variable']
    ...
    for i, msg, cur_state in zip(jq_trace.input(states), jq_msgs.input(states), states):
        ...
        if len(i) > 1:
            ...
            if cmd == 'conn_part_flush':
                ...
                if comment[0] == 'HandleMsgRVR':
                    yield ['sleep', '1000']
                elif comment[0] == 'HandleRequestVoteResponse':
                    yield ['sleep', '1000']
                elif comment[0] == 'HandleAppendEntriesRequest':
                    yield ['sleep', '1000']
                elif comment[0] == 'AppendEntriesAlreadyDone':
                    yield ['sleep', '1000']
                elif comment[0] == 'DropStaleResponse':
                    yield ['sleep', '1000']
                ...
                if comment[0] == 'SendPendingMessages':
                    yield ['intercept', comment[1], 'inc_time_ms', '100']   # skip backoff time
                yield ['sleep', '2000']
                ...
                yield ['intercept', comment[1], 'inc_time_ms', '1500']  # < RequestTimeout
            elif cmd == 'Restart':
                yield ['sleep', '100']
            elif cmd == 'RequestVote':
                yield ['sleep', '100']
            elif cmd == 'AppendEntriesInRangeToPeer':
                yield ['sleep', '100']
            elif cmd == 'AppendEntriesToSelf':
                yield ['sleep', '100']
            elif cmd == 'BecomeLeader':
                yield ['sleep', '100']
            elif cmd == 'AdvanceCommitIndex':
                yield ['sleep', '100']
            elif cmd == 'BecomeFollowerOfTerm':
                yield ['sleep', '100']
            elif cmd == 'ConflictAppendEntriesRequest':
                yield ['sleep', '100']
            elif cmd == 'NoConflictAppendEntriesRequest':
                yield ['sleep', '100']
        else:
            if comment[0] == 'init':
                yield ['init', str(i[0][1])]
                yield ['sleep', '30000']  # sleep 5 seconds to wait nodes init
                # for j in nodes:
                #     yield ['intercept', j, 'inc_time_ms', '20000' ]
                # for j in nodes:
